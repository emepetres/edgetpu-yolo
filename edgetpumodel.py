...
class EdgeTPUModel:
    ...
    def annotate_image(
        self, det, output_image, pad, hide_labels=False, hide_conf=False, quiet=True
    ):
        """
        Output an image with annotations
        """
        if not len(det):
            return None
        ...
        for *xyxy, conf, cls in reversed(det):
            ...
            label = (
                None
                if hide_labels
                else (self.names[c] if hide_conf else f"{self.names[c]} {conf:.2f}")
            )
